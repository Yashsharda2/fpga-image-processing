...
parameter TOTAL_APPS        = `TOTAL_APPS + 1;  // = 2 (slot0=version, slot1=MY_APP)

// ----------------------------------------------------------------
// All wire declarations up front
// ----------------------------------------------------------------
...
// ----------------------------------------------------------------
// Slot 0 — RAH version check (mandatory, always present)
// ----------------------------------------------------------------
...
cam_rx #(.RAH_PACKET_WIDTH(RAH_PACKET_WIDTH)) u_cam_rx (
    .clk   (rx_pixel_clk),
    .a     (`GET_DATA_RAH(`CAM_RX)),
    .empty (data_queue_empty[`CAM_RX]),
    .c     (`SET_DATA_RAH(`CAM_RX)),
    .rden  (request_data[`CAM_RX]),
    .wren  (write_apps_data[`CAM_RX])
);
